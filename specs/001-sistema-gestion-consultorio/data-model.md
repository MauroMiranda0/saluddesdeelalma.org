...
El modelo prioriza una sola cuenta administradora en el MVP, perfiles clínicos sin login, continuidad paciente-psicóloga y trazabilidad completa de accesos, citas, pagos, recordatorios y mensajería administrativa por WhatsApp.
...
| assigned_therapist_id | UUID                        | No       | FK -> therapist_profiles.id; requerido antes de crear una cita nueva  |
| created_at            | timestamptz                 | Yes      | Default now                                                           |
| updated_at            | timestamptz                 | Yes      | Default now                                                           |
...
| --------- | ------- | -------- | ---------------------------------------------------- |
| id        | UUID    | Yes      | PK                                                   |
| user_id   | UUID    | Yes      | Unique FK -> users.id; perfil clínico, no credencial |
| is_active | boolean | Yes      | Default `true`                                       |

Solo `admin` crea, activa, asigna o reasigna estos perfiles. Reasignar un paciente conserva las citas históricas y deja auditoría.
...
| therapist_id        | UUID                                                     | Yes para nuevas citas | FK -> therapist_profiles.id; copia de la asignación vigente del paciente |
| scheduled_at        | timestamptz                                              | Yes                   | Fecha/hora de sesion                                                     |
| ends_at             | timestamptz                                              | Yes para nuevas citas | `scheduled_at + duration_minutes`                                        |
| therapy_type        | enum(`individual`,`pareja`,`familiar`)                   | Yes para nuevas citas | Individual dura 60 min; pareja y familiar 90 min                         |
| duration_minutes    | integer                                                  | Yes para nuevas citas | Derivado por el servidor: 60 o 90; no editable libremente                |
...
- Restricción PostgreSQL `EXCLUDE` por `therapist_id` y rango `[scheduled_at, ends_at)` donde `status IN ('programada','confirmada')`. Bloquea traslapes online/presencial de una misma psicóloga y permite citas consecutivas.
- Validación de horario regular: lunes a viernes de 09:00 a 21:00 y el fin de la sesión debe estar dentro del horario salvo `is_manual_exception = true`.
...
| id                  | UUID                                                                                              | Yes      | PK                                                                                          |
| appointment_id      | UUID                                                                                              | Yes      | FK -> appointments.id                                                                       |
| reminder_type       | enum(`confirmacion`,`recordatorio_24h`,`cancelacion`,`pago_pendiente`,`pago_pendiente_post_cita`) | Yes      |                                                                                             |
| recipient           | enum(`paciente`,`grupo_psicologas`)                                                               | Yes      | El destino grupal es interno y no recibe datos de pago o clínicos                           |
...
- El servicio crea dos registros `recordatorio_24h`: uno para `paciente` y otro para `grupo_psicologas`, ambos para la ventana 18:00–19:00 de `America/Mexico_City` del día previo. El aviso previo `pago_pendiente` solo se entrega al paciente si mantiene saldo.
...
- `therapist_profiles 1-N patients` (asignación vigente)
- `therapist_profiles 1-N appointments` (asignación histórica de la cita)
...
- Solo citas activas (`programada`, `confirmada`) bloquean el intervalo completo de la psicóloga, independientemente de modalidad.
...
- `daily_agenda_view`: citas del dia con paciente, psicóloga, tipo, duración, inicio/fin, modalidad, estado y resumen de pago.
